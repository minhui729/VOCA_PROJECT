        ...
        const fetchReport = async () => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    setError('학습 리포트를 불러오는 데 실패했습니다.');
                }
            } catch (err) {
...
const StudentManagementPanel = () => {
  ...
  const fetchStudents = useCallback(async () => {
    ...
    setError('');
    try {
      const response = await fetch('http://127.0.0.1:8000/api/teacher/students/', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (response.ok) {
        const data = await response.json();
  ...
  const handleResetPassword = async () => {
    ...
    setError('');
    try {
        const response = await fetch(`http://127.0.0.1:8000/api/users/${studentToReset.id}/reset-password`, {
            method: 'PUT',
            headers: { 'Authorization': `Bearer ${token}` }
        });
        if (response.ok) {
            ...
            {students.map(student => (
              <tr key={student.id}>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <button onClick={() => setViewingStudent(student)} className="text-blue-600 dark:text-blue-400 hover:underline">
                        {student.name}
                    </button>
                </td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl p-8 w-full max-w-md m-4">
            <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={handleCloseAddModal} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200">
                    <X size={24} />
                </button>
            </div>
            {!submissionResult ? (
                <form onSubmit={handleAddStudent} className="space-y-4">
                    ...
                    <div className="pt-4">
                        <button type="submit" disabled={isSubmitting} className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                ...
                <div className={`p-4 rounded-md ${submissionResult.success ? 'bg-green-50 dark:bg-green-900/20' : 'bg-red-50 dark:bg-red-900/20'}`}>
                    ...
                    <button onClick={handleCloseAddModal} className="mt-4 w-full text-center px-4 py-2 bg-gray-200 dark:bg-gray-600 rounded-md hover:bg-gray-300 dark:hover:bg-gray-500">
                        닫기
                    </button>
                </div>
            )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl p-8 w-full max-w-md m-4">
                ...
                <p className="mt-4 text-gray-600 dark:text-gray-300">
                    정말로 <span className="font-bold text-red-500">{studentToDelete.name} ({studentToDelete.username})</span> 학생을 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.
                </p>
                <div className="mt-6 flex justify-end gap-4">
                    ...
                    <button onClick={handleDeleteStudent} disabled={isDeleting} className="px-4 py-2 bg-red-600 text-white font-semibold rounded-lg shadow-md hover:bg-red-700 disabled:bg-red-400 flex items-center gap-2">
                        {isDeleting ? <Loader2 className="animate-spin" /> : <Trash2 size={18} />}삭제 확인
                    </button>
                </div>
            </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl p-8 w-full max-w-md m-4">
                ...
                {resetResult ? (
                    ...
                    <div>
                        <p className="mt-4 text-gray-600 dark:text-gray-300">
                            정말로 <span className="font-bold text-indigo-500">{studentToReset.name} ({studentToReset.username})</span> 학생의 비밀번호를 초기화하시겠습니까?
                        </p>
                        <div className="mt-6 flex justify-end gap-4">
                            ...
                            <button onClick={handleResetPassword} disabled={isResetting} className="px-4 py-2 bg-indigo-600 text-white font-semibold rounded-lg shadow-md hover:bg-indigo-700 disabled:bg-indigo-400 flex items-center gap-2">
                                {isResetting ? <Loader2 className="animate-spin" /> : <KeyRound size={18} />}초기화
                            </button>
                        </div>
                    </div>
                )}
...
const WordbookUploadPanel = () => {
  ...
  const handleFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile) {
      ...
      Papa.parse(selectedFile, {
        ...
        skipEmptyLines: true,
        complete: (result) => {
          const parsedData: ParsedWord[] = (result.data as string[][])
            .map(row => ({
              text: row[0]?.trim(),
              meaning: row[1]?.trim(),
            ...
            .filter(word => word.text && word.meaning);
          setWords(parsedData);
        },
        error: (err) => {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (selectedStudentIds.length === 0) {
      setError('단어장을 할당할 학생을 한 명 이상 선택해주세요.');
      return;
    }
    if (!token) {
      setError('로그인이 필요합니다.');
            ...
            <div className="max-h-48 overflow-y-auto p-3 bg-gray-100/50 dark:bg-gray-700/50 rounded-lg space-y-2 border border-gray-200 dark:border-gray-700">
              {allStudents.map(student => (
                <div key={student.id} className="flex items-center">
                  <input
                    type="checkbox"
                    id={`student-upload-${student.id}`}
                    checked={selectedStudentIds.includes(student.id)}
                    onChange={() => handleStudentSelect(student.id)}
                    className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                  />
                  <label htmlFor={`student-upload-${student.id}`} className="ml-3 block text-sm text-gray-900 dark:text-gray-200">
        ...
        {words.length > 0 && (
          <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
            <p className="text-sm font-medium text-blue-800 dark:text-blue-200">
              총 <span className="font-bold">{words.length}</span>개의 단어를 <span className="font-bold">{selectedStudentIds.length}</span>명의 학생에게 할당합니다.
            </p>
          </div>
        )}
        <div className="pt-4">
          <button type="submit" disabled={isLoading || words.length === 0 || selectedStudentIds.length === 0} className="w-full flex justify-center items-center gap-2 px-8 py-4 bg-blue-600 text-white font-bold rounded-lg shadow-md hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-all">
            {isLoading ? <><Loader2 className="animate-spin" /> 업로드 중...</> : '업로드하여 단어장 생성'}
          </button>
        </div>
