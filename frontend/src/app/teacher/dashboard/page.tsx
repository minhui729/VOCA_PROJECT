...
const StudentManagementPanel = () => {
  ...
  const handleDeleteStudent = async () => {
    ...
    try {
        const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000';
        const response = await fetch(`${API_BASE_URL}/api/users/${studentToDelete.id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` } });
        if (response.ok) {
  ...
  const handleResetPassword = async () => {
    ...
    try {
        const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000';
        const response = await fetch(`${API_BASE_URL}/api/users/${studentToReset.id}/reset-password`, { method: 'PUT', headers: { 'Authorization': `Bearer ${token}` } });
        if (response.ok) {
    ...
    <div className="p-4 sm:p-6 bg-white dark:bg-gray-800/50 rounded-lg shadow-md">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full bg-white dark:bg-gray-800 rounded-lg">
          <thead className="bg-gray-50 dark:bg-gray-700"><tr><th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">이름</th><th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">아이디</th><th className="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">관리</th></tr></thead>
      ...
      {isAddModalOpen && ( <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50"><div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl p-8 w-full max-w-md m-4"><div className="flex justify-between items-center mb-6"><h3 className="text-2xl font-bold text-gray-800 dark:text-white">신규 학생 등록</h3><button onClick={handleCloseAddModal} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200"><X size={24} /></button></div>{!submissionResult ? (<form onSubmit={handleAddStudent} className="space-y-4"><div><label htmlFor="new-student-name" className="block text-sm font-medium text-gray-700 dark:text-gray-300">이름</label><input type="text" id="new-student-name" value={newStudentName} onChange={(e) => setNewStudentName(e.target.value)} className="mt-1 block w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" required /></div><div><label htmlFor="new-student-username" className="block text-sm font-medium text-gray-700 dark:text-gray-300">아이디</label><input type="text" id="new-student-username" value={newStudentUsername} onChange={(e) => setNewStudentUsername(e.target.value)} className="mt-1 block w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" required /></div><p className="text-xs text-gray-500 dark:text-gray-400">초기 비밀번호는 &apos;1234&apos;로 설정됩니다.</p><div className="pt-4"><button type="submit" disabled={isSubmitting} className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">{isSubmitting ? <Loader2 className="animate-spin" /> : '등록하기'}</button></div></form>) : (<div className={`p-4 rounded-md ${submissionResult.success ? 'bg-green-50 dark:bg-green-900/20' : 'bg-red-50 dark:bg-red-900/20'}`}><div className="flex items-center">{submissionResult.success ? <CheckCircle className="text-green-500" /> : <AlertCircle className="text-red-500" />}<p className={`ml-3 text-sm font-medium ${submissionResult.success ? 'text-green-800 dark:text-green-200' : 'text-red-800 dark:text-red-200'}`}>{submissionResult.message}</p></div><button onClick={handleCloseAddModal} className="mt-4 w-full text-center px-4 py-2 bg-gray-200 dark:bg-gray-600 rounded-md hover:bg-gray-300 dark:hover:bg-gray-500">닫기</button></div>)}</div></div>)}
      {studentToDelete && (<div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50"><div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl p-8 w-full max-w-md m-4"><h3 className="text-2xl font-bold text-gray-800 dark:text-white">학생 삭제 확인</h3><p className="mt-4 text-gray-600 dark:text-gray-300">정말로 <span className="font-bold text-red-500">{studentToDelete.name} ({studentToDelete.username})</span> 학생을 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.</p><div className="mt-6 flex justify-end gap-4"><button onClick={() => setStudentToDelete(null)} disabled={isDeleting} className="px-4 py-2 bg-gray-200 dark:bg-gray-600 rounded-md hover:bg-gray-300 dark:hover:bg-gray-500 disabled:opacity-50">취소</button><button onClick={handleDeleteStudent} disabled={isDeleting} className="px-4 py-2 bg-red-600 text-white font-semibold rounded-lg shadow-md hover:bg-red-700 disabled:bg-red-400 flex items-center gap-2">{isDeleting ? <Loader2 className="animate-spin" /> : <Trash2 size={18} />}삭제 확인</button></div></div></div>)}
      {studentToReset && (<div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50"><div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl p-8 w-full max-w-md m-4"><h3 className="text-2xl font-bold text-gray-800 dark:text-white">비밀번호 초기화</h3>{resetResult ? (<div><div className="flex items-center gap-2 mt-4 p-4 bg-green-50 dark:bg-green-900/20 rounded-md"><CheckCircle className="text-green-500" /><p className="text-green-800 dark:text-green-200">{resetResult}</p></div><button onClick={() => { setStudentToReset(null); setResetResult(null); }} className="mt-6 w-full px-4 py-2 bg-gray-200 dark:bg-gray-600 rounded-md hover:bg-gray-300 dark:hover:bg-gray-500">닫기</button></div>) : (<div><p className="mt-4 text-gray-600 dark:text-gray-300">정말로 <span className="font-bold text-indigo-500">{studentToReset.name} ({studentToReset.username})</span> 학생의 비밀번호를 초기화하시겠습니까?</p><div className="mt-6 flex justify-end gap-4"><button onClick={() => setStudentToReset(null)} disabled={isResetting} className="px-4 py-2 bg-gray-200 dark:bg-gray-600 rounded-md hover:bg-gray-300 dark:hover:bg-gray-500 disabled:opacity-50">취소</button><button onClick={handleResetPassword} disabled={isResetting} className="px-4 py-2 bg-indigo-600 text-white font-semibold rounded-lg shadow-md hover:bg-indigo-700 disabled:bg-indigo-400 flex items-center gap-2">{isResetting ? <Loader2 className="animate-spin" /> : <KeyRound size={18} />}초기화</button></div></div>)}</div></div>)}
    </div>
  );
};
...
const WordbookUploadPanel = () => {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000';
      const response = await fetch(`${API_BASE_URL}/api/wordbooks/upload/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
  ...
  return (
    <div className="p-4 sm:p-6 bg-white dark:bg-gray-800/50 rounded-lg shadow-md">
      <form onSubmit={handleSubmit} className="space-y-6">
        <div><label htmlFor="title-tab" className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">단어장 제목</label><input id="title-tab" type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="예: 중등 필수 영단어 1200" className="w-full p-3 bg-gray-100/50 dark:bg-gray-700/50 rounded-lg border-transparent focus:ring-2 focus:ring-blue-500 transition" required /></div>
        <div><label htmlFor="description-tab" className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">설명 (선택)</label><textarea id="description-tab" value={description} onChange={(e) => setDescription(e.target.value)} rows={3} placeholder="이 단어장에 대한 간단한 설명을 입력하세요." className="w-full p-3 bg-gray-100/50 dark:bg-gray-700/50 rounded-lg border-transparent focus:ring-2 focus:ring-blue-500 transition" /></div>
        <div><label htmlFor="file-upload-tab" className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">단어 목록 CSV 파일</label><input id="file-upload-tab" type="file" onChange={handleFileChange} accept=".csv" className="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" required /><p className="mt-1 text-xs text-gray-500 dark:text-gray-400">파일 형식: 단어,뜻,품사,예문 (품사와 예문은 생략 가능)</p></div>
        <div>
          <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">단어장 할당 학생</label>
          {allStudents.length > 0 ? (<div className="max-h-48 overflow-y-auto p-3 bg-gray-100/50 dark:bg-gray-700/50 rounded-lg space-y-2 border border-gray-200 dark:border-gray-700">{allStudents.map(student => (<div key={student.id} className="flex items-center"><input type="checkbox" id={`student-upload-${student.id}`} checked={selectedStudentIds.includes(student.id)} onChange={() => handleStudentSelect(student.id)} className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500" /><label htmlFor={`student-upload-${student.id}`} className="ml-3 block text-sm text-gray-900 dark:text-gray-200">{student.name} ({student.username})</label></div>))}</div>) : <p className="text-sm text-gray-500 dark:text-gray-400 p-3 bg-gray-100/50 dark:bg-gray-700/50 rounded-lg">등록된 학생이 없습니다.</p>}
        </div>
        {words.length > 0 && (<div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg"><p className="text-sm font-medium text-blue-800 dark:text-blue-200">총 <span className="font-bold">{words.length}</span>개의 단어를 <span className="font-bold">{selectedStudentIds.length}</span>명의 학생에게 할당합니다.</p></div>)}
